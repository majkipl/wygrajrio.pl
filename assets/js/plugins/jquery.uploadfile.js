        ...
        function setDragDropHandlers(obj, s, ddObj) {
            ...
            ddObj.on('drop', function (e) {
                ...
                if (!s.multiple && files.length > 1) {
                    if (s.showError) $("<div class='error-o' style='color:red;'>" + s.multiDragErrorStr + "</div>").appendTo(obj.errorLog);
                    return;
                }
        ...
        function serializeAndUploadFiles(s, obj, files) {
            for (var i = 0; i < files.length; i++) {
                ...
                if (s.maxFileSize != -1 && files[i].size > s.maxFileSize) {
                    if (s.showError) $("<div class='error-o' style='color:red;'>" + "Max. rozmiar pliku to : " + getSizeStr(s.maxFileSize) + "</div>").appendTo(obj.errorLog);
                    continue;
                }
                var ts = s;
                var fd = new FormData();
